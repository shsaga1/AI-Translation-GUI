...
class WebTranslatePage(QWidget):
    CONTEXT_OPTIONS = [
        ("关闭上下文（最快）", 0),
        ("弱上下文（快）", 1),
        ("标准上下文（中等）", 2),
    ...
    def _build_ui(self):
        ...
        input_box = QGroupBox("网页输入")
        input_layout = QGridLayout(input_box)

        self.url_edit = QLineEdit()
        self.url_edit.setPlaceholderText("请输入 URL，例如 https://example.com")

        self.output_edit = QLineEdit()
        self.output_edit.setReadOnly(True)
        self.output_edit.setText(str(self.output_root / "web_output"))

        input_layout.addWidget(QLabel("网页 URL"), 0, 0)
        input_layout.addWidget(self.url_edit, 0, 1, 1, 2)
        ...
        self.start_btn.clicked.connect(self.start_web_translation)
        ...
        self.url_edit.textChanged.connect(self.update_summary)
    ...
    def refresh_models(self):
        ...
        self.log_text.append(f"[INFO] 网页翻译可用模型数量：{len(mt_models)}")
        self.update_summary()

    def update_summary(self):
        self.summary_label.setText(
            f"网页 URL：{self.url_edit.text().strip() or '未填写'}\n"
    ...
    def start_web_translation(self):
        url = self.url_edit.text().strip()
        output_dir = self.output_edit.text().strip()
        mt_model = self.mt_combo.currentData()

        if not url:
            QMessageBox.warning(self, "提示", "请先输入 URL。")
            return
        ...
        self.worker = WebTranslationWorker(payload)
